...
export function ShareCard({ meetupId, title }: { meetupId: string; title: string }) {
  ...
  const share = async () => { if (navigator.share) await navigator.share({ title, text: korean ? `${title}에 같이 와요!` : `${title}に一緒に行こう！`, url }); else await navigator.clipboard.writeText(shareText); complete(); };
