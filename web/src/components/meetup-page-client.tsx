...
export function MeetupPageClient({ meetupId }: { meetupId: string }) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const invite = await getInvitePreview(meetupId);
        setPreview(invite);
        setJoined(invite.isAlreadyParticipant);
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : "초대를 불러올 수 없어요.");
      }
    })();
  }, [meetupId]);
