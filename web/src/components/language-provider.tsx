...
export function AppHeader({ title }: { title: keyof typeof headerTitles }) {
  ...
  return <header className="page-header"><Link className="brand" href="/" aria-label="aimasho home"><span className="brand-mark">a</span><span>aimasho</span></Link><div className="header-controls"><span className="page-context-title">{headerTitles[title][language]}</span>{loading ? <span className="header-auth-loading" aria-label={korean ? "로그인 상태 확인 중" : "ログイン状態を確認中"}>…</span> : signedIn ? <Link className="page-account-link" href="/profile">✓ {user?.displayName || (korean ? "내 계정" : "マイアカウント")}</Link> : title !== "login" ? <Link className="header-login-link" href={loginPath}>{korean ? "로그인 / 회원가입" : "ログイン・新規登録"}</Link> : null}<LanguageSelect /></div></header>;
}
