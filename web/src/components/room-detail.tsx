...
import { deleteRoom, getRoomDetail } from "@/services/meetup-repository";
...
export function RoomDetail({ roomId }: { roomId: string }) {
  ...
  const removeRoom = async () => {
    const confirmed = window.confirm(korean
      ? "이 그룹을 삭제할까요? 그룹과 멤버 정보는 삭제되지만 기존 일정 기록은 유지됩니다."
      : "このグループを削除しますか？グループとメンバー情報は削除されますが、これまでの予定は残ります。");
    if (!confirmed) return;
    setDeleting(true);
    setError(undefined);
    try {
      ...
      setError(caught instanceof Error ? caught.message : korean ? "그룹을 삭제하지 못했어요." : "グループを削除できませんでした。");
      setDeleting(false);
    }
  };
  if (authLoading) return <main className="loading-page"><div className="loader" /><p>{korean ? "로그인 상태를 확인하고 있어요..." : "ログイン状態を確認しています…"}</p></main>;
  if (!user || user.isAnonymous) return <main className="profile-page"><section className="account-card group-room-gate"><div><p className="eyebrow">{korean ? "그룹 멤버 전용" : "グループメンバー限定"}</p><h2>{korean ? "로그인하고 그룹 방을 확인하세요" : "ログインしてグループルームを確認"}</h2><p>{korean ? "로그인하면 멤버, 다음 일정, 함께한 기록을 안전하게 이어볼 수 있어요." : "ログインすると、メンバー・次の予定・一緒に過ごした記録を安全に確認できます。"}</p></div><Link className="primary-button" href={`/login?next=${encodeURIComponent(`/rooms/${roomId}`)}`}>{korean ? "로그인 / 회원가입" : "ログイン・新規登録"}</Link></section></main>;
