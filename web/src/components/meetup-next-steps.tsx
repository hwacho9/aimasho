...
function LocationStep({ meetupId, isHost, onChanged }: { meetupId: string; isHost: boolean; onChanged: () => void }) {
  ...
  const recommend = async (nextMode = mode) => { setMode(nextMode); setLoading(true); setError(undefined); try { setCandidates(await getMeetingPointRecommendations(meetupId, nextMode)); } catch (caught) { setError(caught instanceof Error ? caught.message : korean ? "추천 장소를 찾지 못했어요." : "おすすめの場所を見つけられませんでした。"); } finally { setLoading(false); } };
...
function RoutesStep({ meetupId, detail, isHost, uid, onChanged }: { meetupId: string; detail: MeetupDetail; isHost: boolean; uid?: string; onChanged: () => void }) {
  ...
  const [loading, setLoading] = useState(false); const [error, setError] = useState<string>(); const calculate = async () => { setLoading(true); try { await calculateRoutes(meetupId); onChanged(); } catch (caught) { setError(caught instanceof Error ? caught.message : korean ? "경로를 계산하지 못했어요." : "経路を計算できませんでした。"); } finally { setLoading(false); } };
