...
export function JoinRoomCard({ inviteCode }: { inviteCode: string }) {
  ...
  return <main className="join-card"><div className="mini-confetti">👥</div><p className="eyebrow">{korean ? "초대받았어요" : "招待されています"}</p><h1>{preview.name}</h1><p>{korean ? <><strong>{preview.ownerName}</strong>님이 이 그룹에 초대했어요.<br />다음 약속도 계속 함께할 수 있어요.</> : <><strong>{preview.ownerName}</strong>さんがこのグループに招待しました。<br />次の予定もずっと一緒に楽しめます。</>}</p>{anonymous ? <div className="stack-md"><button className="primary-button" onClick={() => void upgrade()} disabled={busy}>{busy ? korean ? "연결 중..." : "連携中…" : korean ? "Google로 계속하기" : "Googleで続ける"}</button><small>{korean ? "그룹은 계정으로만 참여할 수 있어요." : "グループにはアカウントでのみ参加できます。"}</small></div> : <form className="stack-md" onSubmit={join}><label className="field"><span>{korean ? "표시할 이름" : "表示する名前"}</span><input value={name} onChange={(event) => setName(event.target.value)} required maxLength={60} /></label><button className="primary-button" disabled={busy || !name.trim()}>{busy ? korean ? "참여 중..." : "参加中…" : korean ? "그룹 참여하기" : "グループに参加"}</button></form>}{error && <p className="error-message">{error}</p>}</main>;
}
