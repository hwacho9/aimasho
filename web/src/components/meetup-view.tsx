...
export function MeetupView({ meetupId }: { meetupId: string }) {
  ...
  return <main className="meetup-page">
    ...
    <section className="schedule-section">
      ...
      {confirmed && isHost && <section className="schedule-change-card"><div><p className="eyebrow">{korean ? "호스트 설정" : "ホスト設定"}</p><h3>{korean ? "집합 날짜·시간 변경" : "集合日時を変更"}</h3><p>{korean ? "변경 후 출발 경로와 알림은 새 시간 기준으로 다시 계산해 주세요." : "変更後は出発経路と通知を新しい日時で計算し直してください。"}</p></div><div className="schedule-change-form"><input aria-label={korean ? "집합 날짜와 시간" : "集合日時"} type="datetime-local" value={scheduleInputValue} onChange={(event) => setScheduledTimeInput(event.target.value)} /><button className="secondary-button" type="button" disabled={!scheduleInputValue || busySlot === "schedule-change"} onClick={() => void changeConfirmedTime(scheduleInputValue)}>{busySlot === "schedule-change" ? korean ? "변경 중..." : "変更中…" : korean ? "시간 변경" : "日時を変更"}</button></div></section>}
    </section>
