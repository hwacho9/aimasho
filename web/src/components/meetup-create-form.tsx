...
export function MeetupCreateForm({ roomId, template = "free" }: { roomId?: string; template?: string }) {
  ...
  const hostName = hostNameInput ?? (user?.displayName ?? "");
  const [description, setDescription] = useState("");
  const [durationMinutes, setDurationMinutes] = useState(120);
  const [slots, setSlots] = useState(initialSlots);
  const [collectOrigins, setCollectOrigins] = useState(false);
  ...
  const [foodVote, setFoodVote] = useState(template === "food");
  const [activityVote, setActivityVote] = useState(template === "activity");
  const [allowMultipleContentVotes, setAllowMultipleContentVotes] = useState(false);
  const [allowPlanEditing, setAllowPlanEditing] = useState(true);
  ...
  return <form className="stack-lg quick-create-form" onSubmit={submit}>
    ...
    <details className="social-disclosure"><summary>{korean ? "설명·예상 시간 (선택)" : "説明・予定時間（任意）"}</summary>
